...
def _configure_acceleration(cfg: Dict[str, Any], preferred: str) -> tuple[str, str]:
    ...
    if effective == "cuda":
        ...
        if vram_gb >= 12.0:
            ...
            if bool(kokoro_cfg.get("use_half")):
                logger.debug("Disabling Kokoro half precision due to limited VRAM (%.1f GiB)", vram_gb)
        ...
        if str(kokoro_cfg.get("device", "")).strip().lower() == "cuda":
            kokoro_cfg["device"] = "auto"
        if bool(kokoro_cfg.get("use_half")):
            logger.debug("Disabling Kokoro half precision for CPU mode")
        kokoro_cfg["use_half"] = False
